...
class WSIPatchDataset(Dataset):
    ...
    def __init__(self, wsi_path, mask_path, trans, level, Wt, Ht, vhd, save_dir, image_size=256, step_size=256,
                 normalize=True, flip='NONE', rotate='NONE'):
        self._wsi_path = wsi_path
        self._mask_path = mask_path
        self._save_path = os.path.join(save_dir, os.path.basename(wsi_path))
        self._wt = Wt
        self._ht = Ht
        self._vhd = vhd 
    ...
    def __getitem__(self, coord_idx):
        ...
        img = np.clip(np.array(img) * 255.0 / p, 0, 255).astype(np.uint8)
        Ws, Hs = self._vhd.stain_separate(img)
        img = self._vhd.SPCN(img, Ws, Hs, self._wt, self._ht)
        #cv2.imwrite(self._save_path.replace('.svs', '_{}.png'.format(coord_idx)), cv2.cvtColor(img, cv2.COLOR_RGB2BGR))
        img = self._trans(img)
