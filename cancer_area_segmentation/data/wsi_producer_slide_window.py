...
class WSIPatchDataset(Dataset):
    ...
    def _pre_process(self):
        ...
        self.x_cors, self.y_cors = list(map(lambda x: x//self.rate, np.where(self._mask)))
    ...
    def __getitem__(self, coord_idx):
        ...
        x_mask = int(self.coords[coord_idx][0])*self._step*self.down_samples
        y_mask = int(self.coords[coord_idx][1])*self._step*self.down_samples
        #print(x_mask,y_mask)
        ...
        x = x_mask if int(x_mask + self._step * self.down_samples) < self.X_slide_0 else int(self.X_slide_0 - self._step * self.down_samples)
        y = y_mask if int(y_mask + self._step * self.down_samples) < self.Y_slide_0 else int(self.Y_slide_0 - self._step * self.down_samples)
